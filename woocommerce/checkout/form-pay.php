...
$cp_order = hive_get_cp_order_info($order->get_id());
...
?>
<?php if ( $order->get_payment_method() ==  $wc_hive->id) : ?>

		<input type="hidden" name="cp_order_remaining_time" value="<?php echo esc_html(hive_order_remaining_time($order->get_id())); ?>">
		...
		<div class="cp-order-info">
				<ul class="cp-order-info-list">
						...
						<li class="cp-order-info-list-item">
								...
								<strong><?php echo esc_html($cp_order->order_in_crypto . " " . $wc_hive->cryptocurrency_used . " (" . $cp_order->order_total . " " . $cp_order->order_default_currency . ")") ?></strong>
						</li>
				</ul>
		</div>
		...
		<div class="cp-box-wrapper">
				<div class="cp-box-col-1">
						<h2><?php echo esc_html($wc_hive->method_title) ?></h2>
						<p class="cp-payment-msg"><?php echo esc_html((hive_order_remaining_time($order->get_id()) < 0) ? "The payment time for order has expired! Do not make any payments as they will be invalid! If you have already made a payment within the allowed time, please wait." : $wc_hive->description) ?></p>
						...
						<div class="cp-payment-info-holder">
								...
								<div class="cp-payment-info">
										...
										<div class="cp-payment-info-text">Exchange rate locked 1 <?php echo esc_html($wc_hive->cryptocurrency_used) ?> = <?php echo esc_html(round($cp_order->order_crypto_exchange_rate, 5) . ' ' . $cp_order->order_default_currency) ?></div>
								</div>
						</div>
				</div>
