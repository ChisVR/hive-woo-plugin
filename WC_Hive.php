...
if (class_exists('WC_Payment_Gateway')) {
    class WC_Hive extends WC_Payment_Gateway{
        ...
        public function __construct(){
            ...
            $this->cryptocurrency_used = "HIVE";
            $this->default_currency_used = get_woocommerce_currency();
            $this->exchange_rate = $this->hive_exchange_rate($this->default_currency_used);
            $this->plugin_version = "1.0.7";

            // Add support for "Woocommerce subscriptions" plugin
            $this->hive_remove_filter( 'template_redirect', 'maybe_setup_cart', 100 );
            $this->supports = array(
               'products',
               'subscriptions',
            );
        ...
        public function init_form_fields(){
                    $this->form_fields = array(
                        ...
                        'description' => array(
                            'title' => __( 'Payment Description', 'woocommerce-hive' ),
                            'type' => 'text',
                            'default' => 'Please send the exact amount in HIVE to the payment address bellow.',
                        ...
                        'confirmation_no' => array(
                         ...
                         'max_time_limit' => array(
                            'title' => __( 'Maximum Payment Time (in Minutes)', 'woocommerce-hive' ),
                            'type' => 'text',
                            'default' => "15",
                ...
                <fieldset style="padding: 0.75em 0.625em 0.75em;">
                    <table>
                        <tr style="vertical-align: middle; text-align: left;">
                            <td width="180">
                                <img alt="plugin logo" width="160" style="max-height: 40px;" src="<?php echo plugins_url('img/plugin-logo-hive.png', __FILE__) ?>">
                            </td>
                            <td>
        ...
        // Remove filters
        function hive_remove_filter( $hook_name = '', $method_name = '', $priority = 0 ) {
            global $wp_filter;
            global $wp;
